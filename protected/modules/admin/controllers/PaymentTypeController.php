...
class PaymentTypeController extends Controller
{
	...
	public function actionCreate()
	{
	    ...
	    $model->updated_on = new CDbExpression('NOW()');
			...
			if($model->save()){
                ...
                Functions::insertEnvironmentContent($_POST['EnvironmentContent'],$id,'1');
	...
	{
		$model=$this->loadModel($id);
        $model2=EnvironmentContent::model()->findAll(array('condition'=>"primary_table_id=$id and primary_table_flag=1"));  
        $model->environments = explode(',',$model->environments);
		...
		if(isset($_POST['PaymentType']))
		{
			...
			if($model->save()){
				$id = $model->getPrimaryKey();
				Functions::updateEnvironmentContent($_POST['EnvironmentContent'],$id,'1');
